    ...
    char checkWin() {
        ...
        for (int i = 0; i < 3; i++) {
            if (grid[i][0] != ' ' && grid[i][0] == grid[i][1] && grid[i][1] == grid[i][2]) {
                winCells = {{i, 0}, {i, 1}, {i, 2}}; return grid[i][0];
            }
            if (grid[0][i] != ' ' && grid[0][i] == grid[1][i] && grid[1][i] == grid[2][i]) {
                winCells = {{0, i}, {1, i}, {2, i}}; return grid[0][i];
            }
        }
        if (grid[0][0] != ' ' && grid[0][0] == grid[1][1] && grid[1][1] == grid[2][2]) {
            winCells = {{0, 0}, {1, 1}, {2, 2}}; return grid[0][0];
        }
        if (grid[0][2] != ' ' && grid[0][2] == grid[1][1] && grid[1][1] == grid[2][0]) {
            winCells = {{0, 2}, {1, 1}, {2, 0}}; return grid[0][2];
        }
        return ' ';
    }
};
...
private:
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                if (b.grid[i][j] == ' ') {
                    ...
                    if (isMax) best = max(best, minimax(b, false));
                    else       best = min(best, minimax(b, true));
                    b.grid[i][j] = ' ';
                }
            }
        }
    ...
    void randomMove(Board &b) {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                if (b.grid[i][j] == ' ') free.push_back({i, j});
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                if (b.grid[i][j] == ' ') {
                    ...
                    b.grid[i][j] = ' ';
                    if (val > bestVal) { bestVal = val; r = i; c = j; }
                }
        if (r != -1 && c != -1) b.grid[r][c] = 'O';
    }

public:
    void playMove(Board &b, Difficulty diff) {
        int chance = rand() % 100;
        if (chance < diff) bestMove(b);
        else randomMove(b);
    }
};
...
public:
    ...
    void reset() {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++) {
                animScale[i][j] = 0; animAlpha[i][j] = 0;
            }
    }
    ...
    void handleClick(Vector2i m) {
        if (state == MENU) {
            if (btnAI.isClicked(m)) { mode = AI_MODE; reset(); }
            else if (btnDiff.isClicked(m)) {
                if (aiDiff == HARD) { aiDiff = EASY; btnDiff.setText("AI: Easy"); }
                else if (aiDiff == EASY) { aiDiff = MEDIUM; btnDiff.setText("AI: Medium"); }
                else { aiDiff = HARD; btnDiff.setText("AI: Hard"); }
            } else if (btnLocal.isClicked(m)) { mode = LOCAL; reset(); }
            ...
            if (btnConnect.isClicked(m)) joinServer();
        } 
            ...
            if (m.x >= BOARD_OFFSET_X && m.x <= BOARD_OFFSET_X + 300 &&
                ...
                if (mode == LOCAL || mode == AI_MODE) canPlayLocal = true;
                else if (mode == NET_HOST && playerTurn) canPlayLocal = true;
                else if (mode == NET_CLIENT && !playerTurn) canPlayLocal = true;

                if (canPlayLocal && y < 3 && x < 3 && board.grid[y][x] == ' ') {
                    board.move(y, x, playerTurn ? 'X' : 'O');
                    if (mode == NET_HOST || mode == NET_CLIENT) {
                        sf::Packet packet; packet << y << x;
                        socket.send(packet);
                    }
            ...
            if (btnMainMenu.isClicked(m)) {
                ...
                listener.close();
                state = MENU;
            } else if (btnPlayAgain.isClicked(m) && result != "Opponent Disconnected.") {
                reset(); // Resets board and state, TCP buffer handles sync automatically
            }
        }
    }
    ...
    void update() {
        ...
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                if (board.grid[i][j] != ' ') {
                    ...
                    if (animScale[i][j] > 1.0f) animScale[i][j] = 1.0f;
                    if (animAlpha[i][j] > 255.0f) animAlpha[i][j] = 255.0f;
                }
        ...
        if ((mode == NET_HOST && !playerTurn) || (mode == NET_CLIENT && playerTurn)) {
            sf::Packet packet;
    ...
    void draw(RenderWindow &w) {
        ...
        bg[0].position = Vector2f(0, 0);               bg[0].color = Color(20, 20, 35);
        bg[1].position = Vector2f(WINDOW_W, 0);        bg[1].color = Color(35, 25, 45);
        bg[2].position = Vector2f(WINDOW_W, WINDOW_H); bg[2].color = Color(10, 10, 20);
        bg[3].position = Vector2f(0, WINDOW_H);        bg[3].color = Color(15, 10, 25);
        w.draw(bg);

        Text title("TIC TAC TOE", font, 36);
        title.setStyle(Text::Bold); title.setFillColor(Color::White);
        ...
        if (state == MENU) {
            ...
            btnAI.update(mouse); btnDiff.update(mouse); btnLocal.update(mouse);
            btnHost.update(mouse); btnJoin.update(mouse);

            btnAI.draw(w); btnDiff.draw(w); btnLocal.draw(w);
            btnHost.draw(w); btnJoin.draw(w);
            return;
        }

        if (state == ENTER_IP) {
            Text prompt("Enter Host IP Address:", font, 20);
            prompt.setPosition(50, 180); w.draw(prompt);
            ...
            ipText.setPosition(60, 225); w.draw(ipText);

            btnConnect.update(Mouse::getPosition(w)); btnConnect.draw(w);
            return;
        }

        if (state == WAITING_CONNECTION) {
            string msg = (mode == NET_HOST) ? "Waiting for connection...\n\nYour LAN IP: " + localIpAddress : "Connecting to " + ipInput + "...";
            Text wait(msg, font, 20);
            wait.setPosition(50, 250); wait.setFillColor(Color(0, 200, 255));
            w.draw(wait);
            return;
        }
        ...
        for (int i = 1; i < 3; i++) {
            line.setPosition(BOARD_OFFSET_X, BOARD_OFFSET_Y + i * CELL_SIZE - 2); w.draw(line);
            line.setSize({4, 300});
            line.setPosition(BOARD_OFFSET_X + i * CELL_SIZE - 2, BOARD_OFFSET_Y); w.draw(line);
            line.setSize({300, 4});
        }
            ...
            for (int j = 0; j < 3; j++) {
                if (board.grid[i][j] != ' ') {
                    ...
                    if (board.grid[i][j] == 'X') {
                        ...
                        xLine.setOrigin(35, 4); xLine.setPosition(cx, cy);
                        xLine.setScale(animScale[i][j], animScale[i][j]);
                        xLine.setFillColor(Color(0, 200, 255, (int)animAlpha[i][j]));
                        xLine.setRotation(45); w.draw(xLine);
                        xLine.setRotation(-45); w.draw(xLine);
                    } else {
                        CircleShape oShape(25);
                        oShape.setOrigin(25, 25); oShape.setPosition(cx, cy);
        ...
        if (board.winCells.size() == 3) {
            Vector2f s(BOARD_OFFSET_X + board.winCells[0].y * CELL_SIZE + 50, BOARD_OFFSET_Y + board.winCells[0].x * CELL_SIZE + 50);
            Vector2f e(BOARD_OFFSET_X + board.winCells[2].y * CELL_SIZE + 50, BOARD_OFFSET_Y + board.winCells[2].x * CELL_SIZE + 50);
            RectangleShape winLine({hypot(e.x - s.x, e.y - s.y) + 40, 8});
            winLine.setOrigin(20, 4); winLine.setPosition(s);
            winLine.setRotation(atan2(e.y - s.y, e.x - s.x) * 180 / 3.14159f);
            winLine.setFillColor(Color(0, 255, 150, 220)); w.draw(winLine);
        }

        if (state == GAME_OVER) {
            RectangleShape overlay({WINDOW_W, WINDOW_H});
            overlay.setFillColor(Color(0, 0, 0, 180)); w.draw(overlay);

            Text res(result, font, 28);
            res.setStyle(Text::Bold); res.setFillColor(Color::White);
            FloatRect rb = res.getLocalBounds();
            res.setOrigin(rb.left + rb.width / 2.0f, rb.top + rb.height / 2.0f);
            res.setPosition(WINDOW_W / 2.0f, WINDOW_H / 2.0f - 80); // Moved up to make room
            w.draw(res);

            // Update and draw the new buttons
            Vector2i mouse = Mouse::getPosition(w);
            btnMainMenu.update(mouse);
            btnMainMenu.draw(w);

            // Only show "Play Again" if the opponent didn't disconnect
            if (result != "Opponent Disconnected.") {
...
int main() {
    ...
    while (window.isOpen()) {
        ...
        while (window.pollEvent(e)) {
            ...
            if (e.type == Event::TextEntered) game.handleText(e.text.unicode);
